...
def hydrogen_production(ww_demand_df, input_path, density, electrolyzer_eff, fuelcell_eff, compressor_req, tank_capacity, case):
    labels = ["Date",
              "E_demand_kWh",
              "E_available_kWh",
              "H2_produced_kg",
    ...
    for name in result_df['Name']:
        with open(fun.get_path_h2(input_path, name, 1), 'w', newline='') as file_SC:
            ...
            h2_produced, h2_tank, h2_demand, fc_demand, elyzer_loss, fc_loss, pb_fc_sold, \
            pb_fc_bought = H2_Storage_Supply(kWh_demand,
                                             kWh_available,
                                             density,
                                             electrolyzer_eff,
                                             fuelcell_eff,
...
def H2_Storage_Supply (demand_kWh, available_kWh, h2_density, Elyzer_eff, FC_eff, Compr_req,  max_capacity):
    ...
    for i in range(len(available_kWh)):
        ...
        try:
            if H2_produced[i] > 0:
                if H2_tank[i-1] + H2_produced[i] - (float(demand_kWh[i]) * (2 - FC_eff / 100)) / (h2_density * FC_eff / 100) > max_capacity:
                    H2_tank.append(round(max_capacity,3))
                    E_fc_sold.append(round(float(available_kWh[i]) - (H2_tank[i] - H2_tank[i-1]) * h2_density * Elyzer_eff / 100, 3))

...
def collector_performance(ww_demand_df, input_path, potential, per_capita, heat_cap, inlet, outlet, max_temp, loss_coef):
    ...
    building_production, building_demand, building_stored, building_request = [], [], [], []
    building_people, building_size, building_area, building_maxstr = [], [], [], []
    building_tank_str, building_ww_cnv_dem, building_ww_str_loss, building_ww_amb_loss = [], [], [], []
    count = 0
    ...
    for name in result_df['Name']:
        ...
        with open(fun.get_path_sc(input_path, name, 1), 'w', newline='') as file_SC:
            ...
            with open(fun.get_path_sc(input_path, name, 3), 'r') as file_SC_ET:
            ...
            Qww_Tank, Qww_conventional, Qww_str_loss, Qww_amb_loss, temp_tnk_C, Q_stored = Calculate_SC_Performance(ww_demand_kWh,
                                                                                                          ww_supply_kWh,
                                                                                                          size,
                                                                                                          area,
                                                                                                          heat_cap,
            ...
            Total_ww_stored = fun.Sum_Stored(Qww_Tank, ww_demand_kWh, Q_stored)
        ...
        building_maxstr.append(round(Q_stored,3))
    ...
    result_df["Qww_Tank_kWh"] = building_tank_str
...
def Calculate_SC_Performance(Qww_demand, Qww_supply, tank, area, capacity, inlet, outlet, max_temp, temp_ext, loss_coef):
    WaterTank, Qww_useFull, Qww_conventional, Qww_str_loss, Qww_amb_loss, temp_tank = [], [], [], [], [], []
        ...
        if float(Qww_demand[i]) > 0:
            try:
                ...
                if float(Qww_demand[i]) > float(Qww_supply[i]):
                    ...
                    Qww_conventional.append(round(float(Qww_demand[i]) - float(Qww_supply[i]), 3))
                    Qww_str_loss.append(0.0)
            ...
            try:
                if float(Qww_supply[i]) > 0:
                    if float(WaterTank[i - 1]) + float(Qww_supply[i]) - Qww_amb_loss[i - 1] > Qmax_stored:
                        WaterTank.append(Qmax_stored)
                        Qww_conventional.append(0.0)
                        ...
                        WaterTank.append(round(float(WaterTank[i - 1]) + float(Qww_supply[i]),3))
                        Qww_conventional.append(0.0)
                        Qww_str_loss.append(0.0)
                        Qww_amb_loss.append(round(fun.ambient_Loss(temp_tank[i-1], temp_ext[i], area, loss_coef), 3))
                        temp_tank.append(
    ...
    return WaterTank, Qww_conventional, Qww_str_loss, Qww_amb_loss, temp_tank, Qmax_stored
    pass
